  ...
  namespace core {
      ...
      a1_(a1),t0_(t0),t1_(t1),tv_(tv),t2_(t2),vLim_(vLim)
    {
      ...
      for(size_t i = 0 ; i < 3 ; i++){
        ...
        assert(t0[i] > -Eigen::NumTraits<value_type>::dummy_precision () && "Duration of the phases in kinodynamicPath must be positives.");
        assert(t1[i] > -Eigen::NumTraits<value_type>::dummy_precision () && "Duration of the phases in kinodynamicPath must be positives.");
        assert(tv[i] > -Eigen::NumTraits<value_type>::dummy_precision () && "Duration of the phases in kinodynamicPath must be positives.");
        assert(t2[i] > -Eigen::NumTraits<value_type>::dummy_precision () && "Duration of the phases in kinodynamicPath must be positives.");
      }
